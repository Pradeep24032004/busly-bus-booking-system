...
# from db import users_col, bookings_col, topup_requests_col, buses_col, routes_col, passengers_col
...
#       - id, reservation_id, bus_id, total_price, status, created_at
#       - route: { src, dst }
#       - bus_start_time
#       - seats: [ "A1", "A2" ]
#       - passengers: [ { seat_number, name, email, mobile } ]
#     Handles user_id stored as ObjectId or string in bookings_col.
#     """
#     if not user:
#         raise HTTPException(status_code=401, detail="Unauthenticated")

#     uid = user.get("_id")
#     # Build OR query to match both ObjectId and string user_id storage shapes
#     q_or = []
...
#         bus_id_str = None
#         # bus_id may be stored as ObjectId or string
#         if isinstance(bus_id, ObjectId):
#             bus_id_str = str(bus_id)
#         elif isinstance(bus_id, str):
#             bus_id_str = bus_id
#         else:
#             # fallback: try to convert reservation bus_id if stored differently in reservation
#             bus_id_str = str(bus_id) if bus_id is not None else None

#         # Lookup bus document (try ObjectId then string)
#         bus_doc = None
#         try:
...
#             # bus start_time (may be datetime)
...
#         # Fetch passengers for this booking (passengers_col uses booking_id stored as ObjectId)
#         seats_list: List[str] = []
#         passengers_list: List[Dict[str, Any]] = []
#         try:
#             # attempt as ObjectId booking id
#             booking_objid = None
...
#             passengers_cursor = None
#             if booking_objid:
#                 passengers_cursor = passengers_col.find({"booking_id": booking_objid})
#             else:
#                 # fallback: booking id stored as string in passengers
#                 passengers_cursor = passengers_col.find({"booking_id": b_id_str})
#             async for p in passengers_cursor:
...
#                 })
#         except Exception:
#             # If passengers lookup fails, continue gracefully
#             seats_list = []
#             passengers_list = []
