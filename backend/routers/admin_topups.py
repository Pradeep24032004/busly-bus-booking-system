...
# async def list_topup_requests(status: Optional[str] = Query(None)):
#     """
#     List top-up requests; filter by status if provided.
#     """
#     q = {}
#     if status:
#         q["status"] = status
...
#         # try to read basic user info
#         user_info = None
#         oid = _to_objectid_if_possible(user_id)
#         if oid:
#             u = await users_col.find_one({"_id": oid})
#             if u:
#                 user_info = {"id": str(u["_id"]), "email": u.get("email"), "name": u.get("name")}
#         if not user_info:
#             # try string id
#             u = await users_col.find_one({"_id": str(user_id)})
#             if u:
#                 user_info = {"id": str(u["_id"]), "email": u.get("email"), "name": u.get("name")}
#         out.append({
#             "id": str(r.get("_id")),
...
# async def approve_topup_request(request_id: str):
#     """
...
#         "status": "approved",
#         "approved_by": "admin",  # you could set admin id if available from require_admin
...
# async def reject_topup_request(request_id: str, payload: Dict[str, Any] = Body(...)):
#     if not ObjectId.is_valid(request_id):
#         raise HTTPException(status_code=400, detail="Invalid request id")
#     rid = ObjectId(request_id)
